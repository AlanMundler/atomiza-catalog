...
// a partir de su ilustración SVG. Se ejecuta automáticamente antes de cada
// `astro build` (ver script "prebuild" en package.json).
...
for (const file of files) {
  ...
  try {
    const bottle = await sharp(path.join(IMG_DIR, file), { density: 300 })
